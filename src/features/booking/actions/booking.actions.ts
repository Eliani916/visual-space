...
export async function createBooking(data: BookingInput) {
  try {
    ...
    if (existingBooking) {
      throw new Error("Jadwal ini sudah dibooking. Silakan pilih jadwal lain.");
    }
    ...
    // Get package price
    const pkg = await prisma.package.findUnique({ where: { id: validatedData.packageId } });
      ...
      data: {
        ...
        payment: {
          create: {
            ...
            amount: validatedData.paymentMethod === "TRANSFER" ? totalPrice * 0.5 : totalPrice, // DP is 50%
            status: "PENDING"
          }
        }
      },
...
export async function getAvailableTimes(dateStr: string) {
  try {
    ...
    const bookedTimes = bookings.map(b => b.bookingTime.substring(0, 5)); // format HH:MM
